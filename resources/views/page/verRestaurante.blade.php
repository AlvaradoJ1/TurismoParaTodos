...
@section('title', '🍽️' . $restaurante->nombre)
@section('contenido')
<script src="{{ asset('js/restauranteMegusta.js') }}" defer></script>

    <div class="sitios-container">
        @php
            $slogans = json_decode($restaurante->slogan, true);
            $slogan = $slogans[app()->getLocale()] ?? 'slogan no disponible';
            $descripciones = json_decode($restaurante->descripcion, true);
    ...
    <div class="sitios-container">
        ...
        <div class="cardC dark-mode-target">
            ...
            @endif
            @foreach ($restaurante->comentarios_restaurantes as $comentario)
                <div class="comments">
            ...
            <div class="text-box dark-mode-target">
                <div class="box-container dark-mode-target">
                    @if (Auth::check())
                        <form action="{{ route('comentarios.store.restaurante', $restaurante->id) }}" method="POST"
                            class="comment-form">
                            @csrf
