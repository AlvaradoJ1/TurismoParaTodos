...
@section('title',$transporte->icono .$transporte->nombre)
@section('contenido')
<script src="{{ asset('js/transporteMegusta.js') }}" defer></script>

<div class="sitios-container">
    @php
        $slogans = json_decode($transporte->slogan, true);
        $slogan = $slogans[app()->getLocale()] ?? 'slogan no disponible';
        $descripciones = json_decode($transporte->descripcion, true);
    ...
    <section class="sitio dark-mode-target" data-lugar="{{ $transporte->ciudad }}">
        ...
        <div class="carousel2 dark-mode-target">
            <div class="image-container2 dark-mode-target">
                <img class="card2 dark-mode-target" src="/img/{{ $img[0] }}" alt="Vista 1">
                <img class="card2 dark-mode-target" src="/img/{{ $img[1] }}" alt="Vista 2">
...
<div class="sitios-container">
    <div class="cardC dark-mode-target">
        ...
        @endif
        @foreach ($transporte->comentarios_transportes as $comentario)
            <div class="comments">
        ...
        <div class="text-box dark-mode-target">
            <div class="box-container dark-mode-target">
                @if (Auth::check())
                    <form action="{{ route('comentarios.store.transporte', $transporte->id) }}" method="POST"
                        class="comment-form">
                        @csrf
